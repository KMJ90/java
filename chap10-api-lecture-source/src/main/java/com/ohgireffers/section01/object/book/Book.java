package com.ohgireffers.section01.object.book;
    ...
    public boolean equals(Object obj) {
        ...
        if (this.title == null) {
            if (other.title != null) {
                ...
                // title 필드가 noll 이 아닌경우
                // 두 인스턴스의 title 필드값이 같아야 한다
                // 그렇지 않은 경우 false 반환

                // String 클래스의 equals() 메소드를 이미 오버라이딩 해 두었기 때문에
                // 문자열의 내용이 같은지 비교하도록 재정의 되어
                // this.title 과 other.title 의 문자열이 같은 내용을 가지고 있는지를 확인한다
                return false;
            }
        ...
        public int hashCode () {
            ...
            // 31이 통상적이 관례이며 String 클래스의 hashCode 에서도 사용한 값이다
            final int PRICE = 31;
            // 31과 필드값을 이용하여 새로운 hashCode 를 연산한다
            // 여기서 포인트는 필드값이 같은 경우 동일한 hashCode 를 반환하면 된다
            result = PRICE * result + this.number;

            // String 클래스의 hashCode 메소드는 이미 재정의 되어있다
            // 같은 값을 가지는 문자열은 동일한 hashCode 값을 반환한다
            result = PRICE * result + this.title.hashCode();
            result = PRICE * result + this.author.hashCode();
            result = PRICE * result + this.price;
