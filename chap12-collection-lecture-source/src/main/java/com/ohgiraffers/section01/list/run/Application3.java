...
public class Application3 {
    public static void main(String[] args) {
        ...
        linkedList.remove(1);  // 인덱스 1번 바나나 삭제

        for (String s : linkedList) {
            System.out.println(s);
        }

        linkedList.set(0,"pineapple");  // 인덱스 0번을 파인에플로 변경

        System.out.println(linkedList);

        System.out.println(linkedList.isEmpty());

        linkedList.clear();  // 인덱스 모든 메소드 제거

        System.out.println(linkedList.isEmpty());  // isEmpty() 인덱스가 비워져 있으면 true, 채워져 있으면 false 반환
    }
}
